...
describe('branch.js', () => {
  ...
  describe('createBranch', () => {
    it('should create new branch with correct parameters', async () => {
      ...
      const result = await createBranch(userOctokit, repoDetails, newBranchName);

      // Should first get the main branch to get its SHA
      expect(userOctokit.request).toHaveBeenNthCalledWith(1,
  ...
  describe('getBranch', () => {
    ...
    it('should throw wrapped error on API failure', async () => {
      ...
      await expect(
        getBranch(userOctokit, repoFullname, ref)
      ).rejects.toThrow('Error occurred while getting branch nonexistent-branch');
    });
    ...
    it('should handle permission errors', async () => {
      ...
      await expect(
        getBranch(userOctokit, repoFullname, ref)
      ).rejects.toThrow('Error occurred while getting branch secure-branch');
    });
  });
  ...
  describe('Error Propagation', () => {
    it('should preserve original error information in createBranch', async () => {
      ...
      originalError.response = { data: { message: 'Reference already exists' } };
    ...
    it('should preserve original error information in getBranch', async () => {
      ...
      try {
        ...
        expect(error.message).toBe('Error occurred while getting branch missing-branch');
        expect(error.cause).toBe(originalError);
      }
    });
